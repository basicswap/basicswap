...
from basicswap.interface.xhv import (
    XHV_COIN,
)
...
from bin.basicswap_run import startDaemon, startXhvDaemon
...
XHV_BASE_RPC_PORT = 21752
XHV_BASE_ZMQ_PORT = 22752
XHV_BASE_WALLET_RPC_PORT = 23792
...
def prepareXhvDataDir(datadir, node_id, conf_file):
    node_dir = os.path.join(datadir, 'xhv_' + str(node_id))
    ...
    with open(cfg_file_path, 'w+') as fp:
        ...
        # fp.write('offline=1\n')
        fp.write('p2p-bind-port={}\n'.format(XHV_BASE_P2P_PORT + node_id))
        fp.write('rpc-bind-port={}\n'.format(XHV_BASE_RPC_PORT + node_id))
        fp.write('p2p-bind-ip=127.0.0.1\n')
        fp.write('rpc-bind-ip=127.0.0.1\n')
        fp.write('prune-blockchain=1\n')
        fp.write('zmq-rpc-bind-port={}\n'.format(XHV_BASE_ZMQ_PORT + node_id))
        fp.write('zmq-rpc-bind-ip=127.0.0.1\n')

        for i in range(0, NUM_XHV_NODES):
            if node_id == i:
                continue
            fp.write('add-exclusive-node=127.0.0.1:{}\n'.format(XHV_BASE_P2P_PORT + i))


def startXhvWalletRPC(node_dir, bin_dir, wallet_bin, node_id, opts=[]):
    daemon_bin = os.path.expanduser(os.path.join(bin_dir, wallet_bin))

    data_dir = os.path.expanduser(node_dir)
    args = [daemon_bin]
    args += ['--non-interactive']
    args += ['--daemon-address=127.0.0.1:{}'.format(XHV_BASE_RPC_PORT + node_id)]
    args += ['--no-dns']
    args += ['--rpc-bind-port={}'.format(XHV_BASE_WALLET_RPC_PORT + node_id)]
...
def prepare_swapclient_dir(datadir, node_id, network_key, network_pubkey, with_coins=set(), cls=None):
    ...
    if Coins.XHV in with_coins:
        settings['chainclients']['haven'] = {
            'connection_type': 'rpc',
            'manage_daemon': False,
            'rpcport': XHV_BASE_RPC_PORT + node_id,
            'walletrpcport': XHV_BASE_WALLET_RPC_PORT + node_id,
            'walletrpcuser': 'test' + str(node_id),
            'walletrpcpassword': 'test_pass' + str(node_id),
            'walletfile': 'testwallet',
            'datadir': os.path.join(datadir, 'xhv_' + str(node_id)),
            'bindir': cfg.XHV_BINDIR,
        }
...
def waitForXHVNode(rpc_offset, max_tries=7):
    for i in range(max_tries + 1):
        try:
            callrpc_xhv(XHV_BASE_RPC_PORT + rpc_offset, 'get_block_count')
            return
        except Exception as ex:
            if i < max_tries:
                logging.warning('Can\'t connect to XHV RPC: %s. Retrying in %d second/s.', str(ex), (i + 1))
                time.sleep(i + 1)
    raise ValueError('waitForXHVNode failed')


def waitForXHVWallet(rpc_offset, auth, max_tries=7):
    for i in range(max_tries + 1):
        try:
            callrpc_xhv(XHV_BASE_WALLET_RPC_PORT + rpc_offset, 'get_languages', auth=auth)
            return
        except Exception as ex:
            if i < max_tries:
                logging.warning('Can\'t connect to XHV wallet RPC: %s. Retrying in %d second/s.', str(ex), (i + 1))
                time.sleep(i + 1)
    raise ValueError('waitForXHVWallet failed')
...
class BaseTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            if cls.start_xhv_nodes:
                for i in range(NUM_XHV_NODES):
                    if not cls.restore_instance:
                        prepareXhvDataDir(TEST_DIR, i, 'havend.conf')

                    cls.xhv_daemons.append(startXhvDaemon(os.path.join(TEST_DIR, 'xhv_' + str(i)), cfg.XHV_BINDIR, cfg.XHVD))
                    logging.info('Started %s %d', cfg.XHVD, cls.xhv_daemons[-1].pid)
                    waitForXHVNode(i)

                    cls.xhv_daemons.append(startXhvWalletRPC(os.path.join(TEST_DIR, 'xhv_' + str(i)), cfg.XHV_BINDIR, cfg.XHV_WALLET_RPC, i))

                for i in range(NUM_XHV_NODES):
                    cls.xhv_wallet_auth.append(('test{0}'.format(i), 'test_pass{0}'.format(i)))
                    logging.info('Creating XHV wallet %i', i)

                    waitForXHVWallet(i, cls.xhv_wallet_auth[i])

                    if not cls.restore_instance:
                        cls.callxhvnodewallet(cls, i, 'create_wallet', {'filename': 'testwallet', 'language': 'English'})
                    cls.callxhvnodewallet(cls, i, 'open_wallet', {'filename': 'testwallet'})
            ...
            for i in range(NUM_NODES):
                ...
                sc.start()
                if cls.start_xhv_nodes:
                    # Set XHV main wallet address
                    xhv_ci = sc.ci(Coins.XHV)
                    sc.setStringKV('main_wallet_addr_' + xhv_ci.coin_name().lower(), xhv_ci.getMainWalletAddress())
            ...
            if cls.restore_instance:
                ...
                if cls.start_xhv_nodes:
                    cls.xhv_addr = cls.callxhvnodewallet(cls, 1, 'get_address')['address']
            else:
                ...
                if cls.start_xhv_nodes:
                    cls.xhv_addr = cls.callxhvnodewallet(cls, 1, 'get_address')['address']
                    if callrpc_xhv(XHV_BASE_RPC_PORT + 1, 'get_block_count')['count'] < num_blocks:
                        logging.info('Mining %d Haven blocks to %s.', num_blocks, cls.xhv_addr)
                        callrpc_xhv(XHV_BASE_RPC_PORT + 1, 'generateblocks', {'wallet_address': cls.xhv_addr, 'amount_of_blocks': num_blocks})
                    logging.info('XHV blocks: %d', callrpc_xhv(XHV_BASE_RPC_PORT + 1, 'get_block_count')['count'])
    ...
    def coins_loop(cls):
        ...
        if cls.xhv_addr is not None:
            callrpc_xhv(XHV_BASE_RPC_PORT + 1, 'generateblocks', {'wallet_address': cls.xhv_addr, 'amount_of_blocks': 1})
    ...
    def callxhvnodewallet(self, node_id, method, params=None):
        return callrpc_xhv(XHV_BASE_WALLET_RPC_PORT + node_id, method, params, auth=self.xhv_wallet_auth[node_id])

    def getXhvBalance(self, js_wallets):
        return float(js_wallets[Coins.XHV.name]['unconfirmed']) + float(js_wallets[Coins.XHV.name]['balance'])
...
class Test(BaseTest):
    ...
    def test_010_txn_size(self):
        ...
        pi = swap_clients[0].pi(SwapTypes.XHV_SWAP)
        ...
        expect_vsize: int = ci.xhv_swap_a_lock_spend_tx_vsize()
        ...
        expect_vsize: int = ci.xhv_swap_b_lock_spend_tx_vsize()
        assert (expect_vsize >= lock_tx_b_spend_decoded['vsize'])
        assert (expect_vsize - lock_tx_b_spend_decoded['vsize'] < 10)

    def test_010_xhv_txn_size(self):
        logging.info('---------- Test {} txn_size'.format(Coins.XHV))

        swap_clients = self.swap_clients
        ci = swap_clients[1].ci(Coins.XHV)
        pi = swap_clients[1].pi(SwapTypes.XHV_SWAP)
        ...
        expect_size: int = ci.xhv_swap_b_lock_spend_tx_vsize()
        assert (expect_size >= actual_size)
        assert (expect_size - actual_size < 10)

    def test_01_part_xhv(self):
        logging.info('---------- Test PART to XHV')
        swap_clients = self.swap_clients

        start_xhv_amount = self.getXhvBalance(read_json_api(1800, 'wallets'))
        js_1 = read_json_api(1801, 'wallets')
        assert (self.getXhvBalance(js_1) > 0.0)

        offer_id = swap_clients[0].postOffer(Coins.PART, Coins.XHV, 100 * COIN, 0.11 * XHV_COIN, 100 * COIN, SwapTypes.XHV_SWAP)
        wait_for_offer(test_delay_event, swap_clients[1], offer_id)
        offers = swap_clients[1].listOffers(filters={'offer_id': offer_id})
        assert (len(offers) == 1)
        offer = offers[0]

        bid_id = swap_clients[1].postXhvBid(offer_id, offer.amount_from)

        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.BID_RECEIVED)

        bid, xhv_swap = swap_clients[0].getXhvBid(bid_id)
        assert (xhv_swap)

        swap_clients[0].acceptXhvBid(bid_id)

        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.SWAP_COMPLETED, wait_for=180)
        wait_for_bid(test_delay_event, swap_clients[1], bid_id, BidStates.SWAP_COMPLETED, sent=True)

        js_0_end = read_json_api(1800, 'wallets')
        end_xhv_amount = self.getXhvBalance(js_0_end)
        xhv_amount_diff = end_xhv_amount - start_xhv_amount
        assert (xhv_amount_diff > 10.9 and xhv_amount_diff < 11.0)
    ...
    def test_02_leader_recover_a_lock_tx(self):
        logging.info('---------- Test PART to XHV leader recovers coin a lock tx')
        swap_clients = self.swap_clients

        offer_id = swap_clients[0].postOffer(
            Coins.PART, Coins.XHV, 101 * COIN, 0.12 * XHV_COIN, 101 * COIN, SwapTypes.XHV_SWAP,
        ...
        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, wait_for=180)
        wait_for_bid(test_delay_event, swap_clients[1], bid_id, [BidStates.BID_STALLED_FOR_TEST, BidStates.XHV_SWAP_FAILED], sent=True)
    ...
    def test_03_follower_recover_a_lock_tx(self):
        logging.info('---------- Test PART to XHV follower recovers coin a lock tx')
        swap_clients = self.swap_clients

        offer_id = swap_clients[0].postOffer(
            Coins.PART, Coins.XHV, 101 * COIN, 0.13 * XHV_COIN, 101 * COIN, SwapTypes.XHV_SWAP,
        ...
        wait_for_bid(test_delay_event, swap_clients[1], bid_id, BidStates.XHV_SWAP_FAILED_SWIPED, wait_for=80, sent=True)
    ...
    def test_04_follower_recover_b_lock_tx(self):
        logging.info('---------- Test PART to XHV follower recovers coin b lock tx')

        swap_clients = self.swap_clients

        offer_id = swap_clients[0].postOffer(
            Coins.PART, Coins.XHV, 101 * COIN, 0.14 * XHV_COIN, 101 * COIN, SwapTypes.XHV_SWAP,
        ...
        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, wait_for=180)
        wait_for_bid(test_delay_event, swap_clients[1], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, sent=True)
    ...
    def test_05_btc_xhv(self):
        logging.info('---------- Test BTC to XHV')
        swap_clients = self.swap_clients
        offer_id = swap_clients[0].postOffer(Coins.BTC, Coins.XHV, 10 * COIN, 100 * XHV_COIN, 10 * COIN, SwapTypes.XHV_SWAP)
        wait_for_offer(test_delay_event, swap_clients[1], offer_id)
        offers = swap_clients[1].listOffers(filters={'offer_id': offer_id})
        offer = offers[0]

        swap_clients[1].ci(Coins.XHV).setFeePriority(3)
        ...
        swap_clients[1].ci(Coins.XHV).setFeePriority(0)
    ...
    def test_06_multiple_swaps(self):
        ...
        offer1_id = swap_clients[0].postOffer(Coins.BTC, Coins.XHV, amt_1, rate_1, amt_1, SwapTypes.XHV_SWAP)
        offer2_id = swap_clients[0].postOffer(Coins.PART, Coins.XHV, amt_2, rate_2, amt_2, SwapTypes.XHV_SWAP)
        ...
        bid1_id = swap_clients[1].postXhvBid(offer1_id, offer1.amount_from)
        bid2_id = swap_clients[1].postXhvBid(offer2_id, offer2.amount_from)

        offer3_id = swap_clients[0].postOffer(Coins.PART, Coins.XHV, 11 * COIN, 0.15 * XHV_COIN, 11 * COIN, SwapTypes.XHV_SWAP)

        wait_for_bid(test_delay_event, swap_clients[0], bid1_id, BidStates.BID_RECEIVED)
        swap_clients[0].acceptXhvBid(bid1_id)

        wait_for_offer(test_delay_event, swap_clients[1], offer3_id)
        offer3 = swap_clients[1].getOffer(offer3_id)
        bid3_id = swap_clients[1].postXhvBid(offer3_id, offer3.amount_from)

        wait_for_bid(test_delay_event, swap_clients[0], bid2_id, BidStates.BID_RECEIVED)
        swap_clients[0].acceptXhvBid(bid2_id)

        wait_for_bid(test_delay_event, swap_clients[0], bid3_id, BidStates.BID_RECEIVED)
        swap_clients[0].acceptXhvBid(bid3_id)
    ...
    def test_08_withdraw(self):
        logging.info('---------- Test XHV withdrawals')
        swap_clients = self.swap_clients
        js_0 = read_json_api(1800, 'wallets')
        address_to = js_0[Coins.XHV.name]['deposit_address']

        js_1 = read_json_api(1801, 'wallets')
        assert (float(js_1[Coins.XHV.name]['balance']) > 0.0)
        ...
        rv = read_json_api(1801, 'wallets/xhv/withdraw', post_json)
        assert (len(rv['txid']) == 64)

    def test_09_auto_accept(self):
        logging.info('---------- Test BTC to XHV auto accept')
        swap_clients = self.swap_clients
        amt_swap = make_int(random.uniform(0.01, 11.0), scale=8, r=1)
        rate_swap = make_int(random.uniform(10.0, 101.0), scale=12, r=1)
        offer_id = swap_clients[0].postOffer(Coins.BTC, Coins.XHV, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP, auto_accept_bids=True)
        wait_for_offer(test_delay_event, swap_clients[1], offer_id)
    ...
    def test_09_1_auto_accept_multiple(self):
        logging.info('---------- Test BTC to XHV auto accept multiple bids')
        swap_clients = self.swap_clients
        ...
        offer_id = swap_clients[0].postOffer(Coins.BTC, Coins.XHV, amt_swap, rate_swap, min_bid, SwapTypes.XHV_SWAP, extra_options=extra_options)
        wait_for_offer(test_delay_event, swap_clients[1], offer_id)
        offer = swap_clients[1].listOffers(filters={'offer_id': offer_id})[0]
    ...
    def test_10_locked_refundtx(self):
        ...
        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.XHV_SWAP_SCRIPT_COIN_LOCKED, wait_for=180)

        bid, xhv_swap = swap_clients[0].getXhvBid(bid_id)
        assert (xhv_swap)

        try:
            swap_clients[0].ci(Coins.BTC).publishTx(xhv_swap.a_lock_refund_tx)
    ...
    def test_11_particl_anon(self):
        ...
        offer_id = swap_clients[0].postOffer(Coins.BTC, Coins.PART_ANON, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP)
        wait_for_offer(test_delay_event, swap_clients[1], offer_id)
        offers = swap_clients[0].listOffers(filters={'offer_id': offer_id})
        offer = offers[0]
    ...
    def test_12_particl_blind(self):
        ...
        amt_swap = make_int(random.uniform(0.1, 2.0), scale=8, r=1)
        rate_swap = make_int(random.uniform(2.0, 20.0), scale=8, r=1)
        offer_id = swap_clients[0].postOffer(Coins.PART_BLIND, Coins.XHV, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP)
        wait_for_offer(test_delay_event, swap_clients[1], offer_id)
        offers = swap_clients[0].listOffers(filters={'offer_id': offer_id})
        offer = offers[0]
    ...
    def test_13_locked_xhv(self):
        logging.info('---------- Test PART to XHV leader recovers coin a lock tx')
        swap_clients = self.swap_clients

        amt_swap = make_int(random.uniform(0.1, 10.0), scale=8, r=1)
        rate_swap = make_int(random.uniform(2.0, 20.0), scale=12, r=1)
        offer_id = swap_clients[0].postOffer(Coins.PART, Coins.XHV, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP)
        ...
        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.BID_RECEIVED)
        swap_clients[1].setBidDebugInd(bid_id, DebugTypes.SEND_LOCKED_XHV)
        swap_clients[0].acceptXhvBid(bid_id)
        ...
        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.XHV_SWAP_SCRIPT_COIN_LOCKED, wait_for=180)
        wait_for_bid(test_delay_event, swap_clients[1], bid_id, BidStates.XHV_SWAP_SCRIPT_COIN_LOCKED, sent=True)
    ...
    def test_14_sweep_balance(self):
        ...
        pi = swap_clients[2].pi(SwapTypes.XHV_SWAP)
        js_w2 = read_json_api(1802, 'wallets')
        ...
        offer_id = swap_clients[2].postOffer(Coins.PART, Coins.XHV, swap_value, 2 * COIN, swap_value, SwapTypes.XHV_SWAP, extra_options=extra_options)
        ...
        bid, _, _, _, _ = swap_clients[2].getXhvBidAndOffer(bid_id)
        assert (bid.xhv_a_lock_tx)
        wtx = ci.rpc('gettransaction', [bid.xhv_a_lock_tx.txid.hex(),])
    ...
    def test_15_missed_xhv_send(self):
        logging.info('---------- Test PART to XHV B lock tx is lost')
        swap_clients = self.swap_clients
        ...
        wait_for_bid(test_delay_event, swap_clients[0], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, wait_for=1800)
        wait_for_bid(test_delay_event, swap_clients[1], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, wait_for=1800, sent=True)

    def test_16_new_subaddress(self):
        logging.info('---------- Test that new subaddresses are created')

        current_subaddress = read_json_api(1800, 'wallets/xhv')['deposit_address']
        first_subaddress = read_json_api(1800, 'wallets/xhv/nextdepositaddr')
        second_subaddress = read_json_api(1800, 'wallets/xhv/nextdepositaddr')
    ...
    def test_97_withdraw_all(self):
        logging.info('---------- Test XHV withdrawal all')

        wait_for_balance(test_delay_event, 'http://127.0.0.1:1800/json/wallets/xhv', 'unconfirmed', 0.0)
        wallets0 = read_json_api(TEST_HTTP_PORT + 0, 'wallets')
        xhv_total = float(wallets0[Coins.XHV.name]['balance'])

        if xhv_total < 10.0:
            address_to = read_json_api(1800, 'wallets')[Coins.XHV.name]['deposit_address']
            post_json = {
                'value': 10.0,
                'address': address_to,
                'sweepall': False,
            }
            json_rv = read_json_api(TEST_HTTP_PORT + 1, 'wallets/xhv/withdraw', post_json)
            wait_for_balance(test_delay_event, 'http://127.0.0.1:1800/json/wallets/xhv', 'balance', 10.0)

        post_json = {
            'address': read_json_api(1801, 'wallets')[Coins.XHV.name]['deposit_address'],
            'sweepall': True,
        }
        json_rv = json.loads(post_json_req('http://127.0.0.1:{}/json/wallets/xhv/withdraw'.format(TEST_HTTP_PORT + 0), post_json))
        assert (len(json_rv['txid']) == 64)

        try:
            logging.info('Disabling XHV mining')
            pause_event.clear()

            address_to = read_json_api(1800, 'wallets')[Coins.XHV.name]['deposit_address']

            wallets1 = read_json_api(TEST_HTTP_PORT + 1, 'wallets')
            xhv_total = float(wallets1[Coins.XHV.name]['balance'])
            assert (xhv_total > 10)
            ...
            json_rv = json.loads(post_json_req('http://127.0.0.1:{}/json/wallets/xhv/withdraw'.format(TEST_HTTP_PORT + 1), post_json))
            assert ('Balance must be fully confirmed to use sweep all' in json_rv['error'])
        finally:
            logging.info('Restoring XHV mining')
            pause_event.set()
