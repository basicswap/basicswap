...
NAV_BINDIR = os.path.expanduser(
    os.getenv("NAV_BINDIR", os.path.join(cfg.DEFAULT_TEST_BINDIR, "navcoin"))
)
...
class Test(TestFunctions):
    ...
    def sync_blocks(cls, wait_for: int = 20, num_nodes: int = 3) -> None:
        ...
        for i in range(wait_for):
            ...
            block_hash0 = callnoderpc(
                0, "getbestblockhash", base_rpc_port=NAV_BASE_RPC_PORT
            )
            matches: int = 0
            for i in range(1, num_nodes):
                block_hash = callnoderpc(
                    i, "getbestblockhash", base_rpc_port=NAV_BASE_RPC_PORT
                )
    ...
    def prepareExtraCoins(cls):
        if cls.restore_instance:
            ...
            nav_addr1 = callnoderpc(
                1, "getnewaddress", ["initial addr"], base_rpc_port=NAV_BASE_RPC_PORT
            )
            nav_addr1 = callnoderpc(
                1, "addwitnessaddress", [nav_addr1], base_rpc_port=NAV_BASE_RPC_PORT
            )
            for i in range(5):
                callnoderpc(
                    0,
    ...
    def coins_loop(cls):
        ...
        chain_height: int = callnoderpc(
            0, "getblockcount", [], base_rpc_port=NAV_BASE_RPC_PORT
        )
        staking_info = callnoderpc(
            0, "getstakinginfo", [], base_rpc_port=NAV_BASE_RPC_PORT
        )
        print(
            "Staking loop: NAV node 0 chain_height {}, staking {}, currentblocktx {}".format(
                chain_height, staking_info["staking"], staking_info["currentblocktx"]
            )
        )
        ...
        try:
            ...
            for i in range(wait_for):
                if test_delay_event.is_set():
                    raise ValueError("Test stopped.")
                if (
                    self.callnoderpc("getblockcount", node_id=node_id)
    ...
    def test_001_segwit(self):
        ...
        addr_witness = self.callnoderpc(
            "addwitnessaddress",
            [
                addr_plain,
            ],
        )
        addr_witness_info = self.callnoderpc(
            "validateaddress",
            [
                addr_witness,
            ],
        ...
        tx_funded_decoded = self.callnoderpc(
            "decoderawtransaction",
            [
                tx_funded,
            ],
        )
        tx_signed_decoded = self.callnoderpc(
            "decoderawtransaction",
            [
                tx_signed,
            ],
    ...
    def test_002_scantxoutset(self):
        ...
        ro = self.callnoderpc(
            "scantxoutset", ["start", ["addr({})".format(addr_witness)]]
        )
    ...
    def test_003_signature_hash(self):
        logging.info(
            "---------- Test {} signature_hash".format(self.test_coin_from.name)
        )
        ...
        tx_wallet_signed = self.callnoderpc(
            "signrawtransaction",
            [
                tx_wallet_funded,
            ],
        )["hex"]
        tx_wallet_decoded = self.callnoderpc(
            "decoderawtransaction",
            [
                tx_wallet_signed,
            ],
        ...
        tx_spend.vin.append(
            CTxIn(
                COutPoint(int(txid, 16), prevout_n),
                scriptSig=script_in,
    ...
    def test_004_csv(self):
        ...
        )  # First generated address has a positive balance
        addr_out = self.callnoderpc(
            "getnewaddress",
            [
                "csv test",
            ],
        ...
        assert (
            addr_out_test in tx_decoded["vout"][utxo_pos]["scriptPubKey"]["addresses"]
        )
        ...
        tx_spend_decoded = self.callnoderpc(
            "decoderawtransaction",
            [
                tx_spend_hex,
            ],
    ...
    def test_007_hdwallet(self):
        ...
        for i, sc in enumerate(self.swap_clients):
            ...
            if i == 0:
                assert (
                    "19ac5fdb423421b7f9a33cf319715742be5f4caa" == ci.getWalletSeedID()
                )
    ...
    def test_05_refund(self):
        ...
        logging.info(
            "---------- Test refund, {} to BTC".format(self.test_coin_from.name)
        )
        swap_clients = self.swap_clients
    ...
    def test_05_bad_ptx(self):
        ...
        logging.info(
            "---------- Test bad PTx, BTC to {}".format(self.test_coin_from.name)
        )
    ...
    def test_06_self_bid(self):
        logging.info(
            "---------- Test same client, BTC to {}".format(self.test_coin_from.name)
        )
    ...
    def test_07_error(self):
        logging.info(
            "---------- Test error, BTC to {}, set fee above bid value".format(
    ...
    def test_02_a_leader_recover_a_lock_tx(self):
        ...
        self.do_test_02_leader_recover_a_lock_tx(
            self.test_coin_from, Coins.XMR, lock_value=5
        )
    ...
    def test_02_b_leader_recover_a_lock_tx_reverse(self):
        ...
        self.prepare_balance(Coins.XMR, 100.0, 1800, 1801)
        self.do_test_02_leader_recover_a_lock_tx(
            Coins.XMR, self.test_coin_from, lock_value=5
        )
