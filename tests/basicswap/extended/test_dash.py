...
DASH_BINDIR = os.path.expanduser(
    os.getenv("DASH_BINDIR", os.path.join(cfg.DEFAULT_TEST_BINDIR, "dash"))
)
...
def dashRpc(cmd, wallet=None):
    return callrpc_cli(
        DASH_BINDIR,
        os.path.join(cfg.TEST_DATADIRS, str(DASH_NODE)),
        "regtest",
        cmd,
        DASH_CLI,
        wallet=wallet,
...
class Test(unittest.TestCase):
    ...
    def test_07_error(self):
        ...
        try:
            ...
            swap_clients[0].getChainClientSettings(Coins.DASH)[
                "override_feerate"
            ] = 10.0
            wait_for_bid(
    ...
    def test_10_prefunded_itx(self):
        ...
        if float(js_w2[tla_from]["balance"]) < 100.0:
            ...
            json_rv = read_json_api(
                1800, "wallets/{}/withdraw".format(tla_from.lower()), post_json
            )
            assert len(json_rv["txid"]) == 64
            wait_for_balance(
                delay_event,
