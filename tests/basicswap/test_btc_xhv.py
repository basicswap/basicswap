...
from .test_xhv import BaseTest, test_delay_event, callnoderpc
...
class TestFunctions(BaseTest):
    ...
    def do_test_01_full_swap(self, coin_from: Coins, coin_to: Coins) -> None:
        ...
        offer_id = swap_clients[id_offerer].postOffer(coin_from, coin_to, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP)
        ...
        bid_id = swap_clients[id_bidder].postXhvBid(offer_id, offer.amount_from)
        ...
        if False:  # TODO: set stakeaddress and xhv rewards to non wallet addresses
    ...
    def do_test_02_leader_recover_a_lock_tx(self, coin_from: Coins, coin_to: Coins, lock_value: int = 32) -> None:
        ...
        offer_id = swap_clients[id_offerer].postOffer(
            coin_from, coin_to, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP,
        ...
        wait_for_bid(test_delay_event, swap_clients[id_leader], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, sent=leader_sent_bid, wait_for=(self.extra_wait_time + 180))
        wait_for_bid(test_delay_event, swap_clients[id_follower], bid_id, [BidStates.BID_STALLED_FOR_TEST, BidStates.XHV_SWAP_FAILED], sent=(not leader_sent_bid), wait_for=(self.extra_wait_time + 30))
    ...
    def do_test_03_follower_recover_a_lock_tx(self, coin_from, coin_to, lock_value: int = 32):
        ...
        wait_for_bid(test_delay_event, swap_clients[id_follower], bid_id, BidStates.XHV_SWAP_FAILED_SWIPED, wait_for=(self.extra_wait_time + 80), sent=(not leader_sent_bid))
    ...
    def do_test_04_follower_recover_b_lock_tx(self, coin_from, coin_to, lock_value: int = 32):
        ...
        leader_sent_bid: bool = True if reverse_bid else False
        wait_for_bid(test_delay_event, swap_clients[id_leader], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, wait_for=(self.extra_wait_time + 200), sent=leader_sent_bid)
        wait_for_bid(test_delay_event, swap_clients[id_follower], bid_id, BidStates.XHV_SWAP_FAILED_REFUNDED, sent=(not leader_sent_bid), wait_for=(self.extra_wait_time + 30))
    ...
    def do_test_05_self_bid(self, coin_from, coin_to):
        ...
        offer_id = swap_clients[id_both].postOffer(coin_from, coin_to, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP, auto_accept_bids=True)
        bid_id = swap_clients[id_both].postXhvBid(offer_id, amt_swap)
...
class BasicSwapTest(TestFunctions):
    ...
    def test_007_hdwallet(self):
        ...
        rv = read_json_api(1800, 'getcoinseed', {'coin': 'XHV'})
    ...
    def test_01_a_full_swap(self):
        if not self.has_segwit:
            return
        self.do_test_01_full_swap(self.test_coin_from, Coins.XHV)

    def test_01_b_full_swap_reverse(self):
        if not self.has_segwit:
            return
        self.prepare_balance(Coins.XHV, 100.0, 1800, 1801)
        self.do_test_01_full_swap(Coins.XHV, self.test_coin_from)
    ...
    def test_02_a_leader_recover_a_lock_tx(self):
        if not self.has_segwit:
            return
        self.do_test_02_leader_recover_a_lock_tx(self.test_coin_from, Coins.XHV)

    def test_02_b_leader_recover_a_lock_tx_reverse(self):
        if not self.has_segwit:
            return
        self.prepare_balance(Coins.XHV, 100.0, 1800, 1801)
        self.do_test_02_leader_recover_a_lock_tx(Coins.XHV, self.test_coin_from)
    ...
    def test_03_a_follower_recover_a_lock_tx(self):
        if not self.has_segwit:
            return
        self.do_test_03_follower_recover_a_lock_tx(self.test_coin_from, Coins.XHV)

    def test_03_b_follower_recover_a_lock_tx_reverse(self):
        if not self.has_segwit:
            return
        self.prepare_balance(Coins.XHV, 100.0, 1800, 1801)
        self.do_test_03_follower_recover_a_lock_tx(Coins.XHV, self.test_coin_from)
    ...
    def test_04_a_follower_recover_b_lock_tx(self):
        if not self.has_segwit:
            return
        self.do_test_04_follower_recover_b_lock_tx(self.test_coin_from, Coins.XHV)

    def test_04_b_follower_recover_b_lock_tx_reverse(self):
        if not self.has_segwit:
            return
        self.prepare_balance(Coins.XHV, 100.0, 1800, 1801)
        self.do_test_04_follower_recover_b_lock_tx(Coins.XHV, self.test_coin_from)
    ...
    def test_05_self_bid(self):
        if not self.has_segwit:
            return
        self.do_test_05_self_bid(self.test_coin_from, Coins.XHV)
    ...
    def test_05_self_bid_rev(self):
        if not self.has_segwit:
            return
        self.do_test_05_self_bid(Coins.XHV, self.test_coin_from)
    ...
    def test_06_preselect_inputs(self):
        ...
        ci_to = swap_clients[2].ci(Coins.XHV)
        ...
        rate_swap = ci_to.make_int(random.uniform(0.2, 20.0), r=1)
        offer_id = swap_clients[2].postOffer(self.test_coin_from, Coins.XHV, swap_value, rate_swap, swap_value, SwapTypes.XHV_SWAP, extra_options=extra_options)
        ...
        wtx = ci.rpc_wallet('gettransaction', [bid.xhv_a_lock_tx.txid.hex(),])
    ...
    def test_07_expire_stuck_accepted(self):
        coin_from, coin_to = (self.test_coin_from, Coins.XHV)
        ...
        offer_id = swap_clients[0].postOffer(coin_from, coin_to, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP, auto_accept_bids=True)
        wait_for_offer(test_delay_event, swap_clients[1], offer_id)
        bid_id = swap_clients[1].postXhvBid(offer_id, amt_swap)
    ...
    def test_08_insufficient_funds(self):
        ...
        offer_id = swap_clients[id_offerer].postOffer(coin_from, coin_to, amt_swap, rate_swap, amt_swap, SwapTypes.XHV_SWAP, auto_accept_bids=True)
        wait_for_offer(test_delay_event, swap_clients[id_bidder], offer_id)

        bid_id = swap_clients[id_bidder].postXhvBid(offer_id, amt_swap)
