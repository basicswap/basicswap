...
XMR_SITE_COMMIT = (
    "d00169a6decd9470ebdf6a75e3351df4ebcd260a"  # Lock hashes.txt to monero version
)
...
BITCOIN_FASTSYNC_FILE = os.getenv(
    "BITCOIN_FASTSYNC_FILE", "utxo-snapshot-bitcoin-mainnet-769818.tar"
)
    ...
    if "win32" in BIN_ARCH or "win64" in BIN_ARCH:
        ...
        with tarfile.open(release_path) as ft:
            for b in bins:
                out_path = os.path.join(bin_dir, b)
                if not os.path.exists(out_path) or extract_core_overwrite:
                    with open(out_path, "wb") as fout, ft.extractfile(
                        get_archive_path(b)
                    ) as fi:
                        fout.write(fi.read())
                    try:
...
def prepareCore(coin, version_data, settings, data_dir, extra_opts={}):
    ...
    with open(assert_path, "rb", 0) as fp, mmap.mmap(
...
def main():
        ...
        try:
            ...
            if not os.path.exists(sync_file_path):
                ...
                if file_size < remote_file.length:
                    logger.warning(
                        f"{BITCOIN_FASTSYNC_FILE} is an unexpected size, {file_size} < {remote_file.length}"
                    )
                    downloadFile(
    ...
    chainclients = {
        "particl": {
            ...
            "manage_daemon": (
                True
                if ("particl" in with_coins and PART_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "bitcoin": {
            ...
            "manage_daemon": (
                True
                if ("bitcoin" in with_coins and BTC_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "litecoin": {
            ...
            "manage_daemon": (
                True
                if ("litecoin" in with_coins and LTC_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "decred": {
            ...
            "manage_daemon": (
                True
                if ("decred" in with_coins and DCR_RPC_HOST == "127.0.0.1")
                else False
            ),
            "manage_wallet_daemon": (
                True
                if ("decred" in with_coins and DCR_WALLET_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "namecoin": {
            ...
            "manage_daemon": (
                True
                if ("namecoin" in with_coins and NMC_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "monero": {
            ...
            "manage_daemon": (
                True
                if ("monero" in with_coins and XMR_RPC_HOST == "127.0.0.1")
                else False
            ),
            "manage_wallet_daemon": (
                True
                if ("monero" in with_coins and XMR_WALLET_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "pivx": {
            ...
            "manage_daemon": (
                True
                if ("pivx" in with_coins and PIVX_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "dash": {
            ...
            "manage_daemon": (
                True
                if ("dash" in with_coins and DASH_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "firo": {
            ...
            "manage_daemon": (
                True
                if ("firo" in with_coins and FIRO_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "navcoin": {
            ...
            "manage_daemon": (
                True
                if ("navcoin" in with_coins and NAV_RPC_HOST == "127.0.0.1")
                else False
            ),
        ...
        "wownero": {
            ...
            "manage_daemon": (
                True
                if ("wownero" in with_coins and WOW_RPC_HOST == "127.0.0.1")
                else False
            ),
            "manage_wallet_daemon": (
                True
                if ("wownero" in with_coins and WOW_WALLET_RPC_HOST == "127.0.0.1")
                else False
            ),
