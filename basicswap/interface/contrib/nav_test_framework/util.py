...
def wait_for_navcoind_start(process, url, i):
    ...
    while runtime > 0:
        if process.poll() is not None:
            raise Exception(
                "navcoind exited with status %i during initialization"
                % process.returncode
            )
        try:
            # print('Checking RPC')
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError(
            "Fee of %s NAV too low! (Should be %s NAV)" % (str(fee), str(target_fee))
        )
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of %s NAV too high! (Should be %s NAV)" % (str(fee), str(target_fee))
        )
