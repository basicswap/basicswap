...
class CAddress(object):
    ...
    def __repr__(self):
        return "CAddress(nServices=%i ip=%s port=%i)" % (
            self.nServices,
            self.ip,
            self.port,
        )
...
class CPartialMerkleTree(object):
    ...
    def __repr__(self):
        return "CPartialMerkleTree(nTransactions=%d vBits.size=%d vHash.size=%d)" % (
            self.nTransactions,
            len(self.vBits),
            len(self.vHash),
        )
...
class CCbTx(object):
    def __init__(
        self, version=None, height=None, merkleRootMNList=None, merkleRootQuorums=None
    ):
        self.set_null()
...
class msg_getmnlistd(object):
    ...
    def __repr__(self):
        return "msg_getmnlistd(baseBlockHash=%064x, blockHash=%064x)" % (
            self.baseBlockHash,
            self.blockHash,
        )
...
class msg_mnlistdiff(object):
    ...
    def __repr__(self):
        return "msg_mnlistdiff(baseBlockHash=%064x, blockHash=%064x)" % (
            self.baseBlockHash,
            self.blockHash,
        )
