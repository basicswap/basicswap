...
def get_rpc_proxy(url, node_number, timeout=None):
    ...
    coverage_logfile = (
        coverage.get_filename(COVERAGE_DIR, node_number) if COVERAGE_DIR else None
    )
...
def sync_blocks(rpc_connections, *, wait=1, timeout=60):
    ...
    while cur_time <= start_time + timeout:
        tips = [
            r.waitforblockheight(maxheight, int(wait * 1000)) for r in rpc_connections
        ]
        if all(t["height"] == maxheight for t in tips):
            if all(t["hash"] == tips[0]["hash"] for t in tips):
                return
            raise AssertionError(
                "Block sync failed, mismatched block hashes:{}".format(
        ...
        cur_time = time.time()
    raise AssertionError(
        "Block sync to height {} timed out:{}".format(
            maxheight, "".join("\n  {!r}".format(tip) for tip in tips)
        )
    )
...
def wait_for_bitcoind_start(process, url, i):
    ...
    while True:
        if process.poll() is not None:
            raise Exception(
                "firod exited with status %i during initialization" % process.returncode
            )
        try:
...
def start_nodes(
    num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None
):
    """
...
def dumpprivkey_otac(node, address):
    ...
    otac_match = re.search(
        "Your one time authorization code is: ([a-zA-Z0-9]+)", error_text["message"]
    )
    if not otac_match:
