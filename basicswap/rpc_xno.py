...
def make_xno_rpc2_func(port, auth, host='127.0.0.1', proxy_host=None, proxy_port=None, default_timeout=120, tag=''):
    port = port
    auth = auth
    host = host
    transport = None
    ...
    def rpc_func(method, params=None, wallet=None, timeout=default_timeout):
        ...
        return callrpc_xno2(port, method, params, auth=auth, rpc_host=host, timeout=timeout, transport=transport, tag=tag)
    return rpc_func


def make_xno_rpc_func(port, auth, host='127.0.0.1', proxy_host=None, proxy_port=None, default_timeout=120, tag=''):
    port = port
    auth = auth
    host = host
    transport = None
    ...
    def rpc_func(method, params=None, wallet=None, timeout=default_timeout):
        nonlocal port, auth, host, transport, tag
        return callrpc_xno(port, method, params, rpc_host=host, auth=auth, timeout=timeout, transport=transport, tag=tag)
    return rpc_func
